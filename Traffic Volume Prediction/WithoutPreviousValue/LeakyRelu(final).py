# -*- coding: utf-8 -*-
"""
Created on Sun Aug  4 20:40:12 2019
@author: anishukla
"""
import numpy as np
import pandas as pd
...
ANN_model2.compile(loss='mean_absolute_error', optimizer='adam', metrics=['accuracy'])

ANN_model2.fit(scaled_x, y_train, epochs=250, batch_size=64, validation_split = 0.3)
...
# Compile the network :
ANN_leaky_relu.compile(loss='mean_absolute_error', optimizer='adam', metrics=['accuracy'])
ANN_leaky_relu.fit(scaled_x_train_prev, y_train, epochs=80, batch_size=64, validation_split = 0.3)
...
submission.to_csv('res26.csv',index=False)
